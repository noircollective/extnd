    ...
    <section ref={containerRef} className="relative min-h-screen w-full bg-[#020202] flex flex-col selection:bg-white selection:text-black overflow-hidden">
      <div className="fixed inset-0 z-0 pointer-events-none">
        <Canvas camera={{ position: [0, 0, 60], fov: 35 }} frameloop="demand">
          ...
          <LiquidBackground />
          <Monolith />
          <Invalidator isVisible={isVisible} />
        </Canvas>
      </div>
      ...
      <div ref={revealRef} className="relative z-10 w-full flex flex-col md:flex-row p-8 md:p-14 lg:p-20 min-h-screen items-center md:items-stretch gap-10">
        <div className="flex-1 min-w-0 flex flex-col justify-between pb-12 md:pb-8 w-full">
          ...
          <button ref={ctaRef} className="w-fit flex items-center gap-4 sm:gap-6 group lg:-translate-y-20">
            ...
            <span className="font-mono text-[11px] font-bold text-white uppercase tracking-[0.2em]">{t("hero.cta")}</span>
          </button>
        </div>

        <div className="w-full md:w-80 lg:w-96 flex-shrink-0 flex flex-col gap-4 justify-center z-20">
          {[
            { id: "001", title: t("hero.availability"), val: t("hero.availabilityVal"), type: "progress" },
            { id: "002", title: t("hero.studioStats"), val: t("hero.studioStatsVal"), type: "data" },
            { id: "003", title: t("hero.expertise"), val: t("hero.expertiseVal"), type: "text" }
          ].map((item) => (
              ...
              {item.type === "progress" ? (
                ...
                <div className="mt-4 flex flex-col gap-3">
                  <div className="flex justify-between text-[10px] font-mono text-white/50">
                    <span>{t("hero.awwwardsTier")}</span>
                    <span>2024-25</span>
                  </div>
                  <div className="h-[1px] w-full bg-white/5" />
                  <div className="flex justify-between text-[10px] font-mono text-white/50">
                    <span>{t("hero.retentionRate")}</span>
                    <span>98.2%</span>
                  </div>
                </div>
              ) : (
                <h3 className="text-sm font-medium text-white/70 mt-3 leading-snug">
                  {t("hero.expertiseDesc")}<span className="italic text-white">{t("hero.expertiseHighlight")}</span>.
                </h3>
              )}
            </div>
          ))}
